...
class ChatService:
    ...
    def process_message(self, user: User, conversation_id: int, content: str) -> tuple[Conversation, str]:
    ...
    def _handle_feedback_turn(
        ...
        follow_up = self._maybe_build_contextual_follow_up(
            conversation=conversation,
            extracted=extracted,
            issue_type=issue.issue_type,
        ...
        if follow_up:
            conversation.state = ConversationState.ISSUE_HANDLING
            return self._respond(conversation, follow_up)

        if self._get_context_value(conversation, "rating") is None:
            conversation.state = ConversationState.ASK_RATING
            return self._respond(conversation, self._build_rating_prompt())

        conversation.state = ConversationState.FEEDBACK_CONTINUE
        return self._respond(conversation, self._build_continue_prompt(conversation, extracted))
    ...
    def _maybe_build_contextual_follow_up(
        ...
        return self._build_follow_up_from_key(follow_up_key)
